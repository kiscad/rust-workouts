...
#[cfg(windows)]
fn path_to_cstring(path: &Path) -> Result<CString> {
    // try to convert to utf8. if this fails, libgit2 cannot handle the path anyway
    match path.to_str() {
...
/// database: a commit, tree, blob, tag, etc. This is a wide hash of the object's contents.
pub struct Oid {
    pub raw: raw::git_oid,
}
...
impl<'text> Signature<'text> {
    ...
    /// or None if it is not well-formed utf8
